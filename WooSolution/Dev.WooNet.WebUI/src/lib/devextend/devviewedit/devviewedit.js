layui.define(['layer', 'table', 'form', 'laydate'], function (exports) {
    ...
    var devviewedit = {
        ...
        render: function (param) {
            setTimeout(function () {
                if (!devviewedit.checkParam(param)) {
                    return;
                }
                else {
                    devviewedit.init(param);
                }
            }, 2000);
        },
        ...
        init: function (param) {
            ...
            if ($(param.elem)) {
                ...
                $(param.elem).parent().append(editEl).append(editbtn);
                $(param.elem).parent().css("width", param.parentWidth-100);
                $(param.elem).css("width", param.parentWidth);
                param.detailwidth = param.parentWidth;
                param.parentWidth = param.parentWidth - 100;
        ...
        checkParam: function (param) {
            if (param.elem == "" || param.elem == undefined) {
                layer.msg('devviewedit控件参数elem不能为空！', { icon: 5 });
                return false;
            }
            if (param.edittype == "" || param.edittype == undefined) {
                layer.msg('devviewedit控件参数edittype不能为空！', { icon: 5 });
                return false;
            }
            if (param.url == "" || param.url == undefined) {
                layer.msg('devviewedit控件参数url不能为空！', { icon: 5 });
                return false;
            }
            if (param.objid == "" || param.objid == undefined) {
                layer.msg('devviewedit控件参数objId不能为空！', { icon: 5 });
                return false;
            }
            if (param.fieldname == "" || param.fieldname == undefined) {
                layer.msg('devviewedit控件参数fieldname不能为空！', { icon: 5 });
                return false;
            }
            if (param.edittype == "selTable") {//列表选择时
                if (param.selobjId == "" || param.selobjId == undefined) {
                    return layer.msg('选择Table时，devviewedit控件参数selobjId不能为空！', { icon: 5 });
                }
    ...
    layui.link(layui.cache.base + '/devextend/devviewedit/devviewedit.css');
    //注册编辑按钮点击事件
    ...
    $('body').on('click', 'a.layui-btn.btn-cancel', function () {
        var rel = $(this).attr('rel');
        var pmhval = JSON.parse($("input[type=hidden][name=h_" + rel + "]").val());
        devviewedit.btncancel(this, pmhval)
    });
    ...
    exports('devviewedit', devviewedit);
