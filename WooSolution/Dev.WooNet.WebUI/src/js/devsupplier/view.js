...
function cleardata(){
    wooutil.devajax({
        type: 'GET',
        url: devsetter.devbaseurl + 'api/DevSupplier/cleardata',
    ...
    form.on('submit(Dev-SubmitSave)', function (data) {
        ...
        if (winui.verifyForm(data.elem)) {
           wooutil.devajax({
            type: 'POST',
            url: devsetter.devbaseurl + 'api/DevSupplier/companySave',
            data: JSON.stringify(postdata),
            dataType: "json",
    ...
    function closeWin() {
        if ($devId > 0) {
            top.winui.window.close('win_viewsupplier');
        } else {
            top.winui.window.close('win_addsupplier');
    ...
    function devSetValues() {
        ...
        if ($devId !== "" && $devId !== undefined) {
            wooutil.devajax({
                type: 'GET',
                url: devsetter.devbaseurl + 'api/DevSupplier/showView',
                //async: false,
                data: { Id: $devId },
                dataType: 'json',
                success: function (res) {
                    form.val("DEV-SupplierForm", res.data);
                    $("#Id").val(res.data.Id);
                },
                error: function (xml) {
                    msg('加载失败!');
...
devbrowse:function(){//预览
    var checkStatus = table.checkStatus("Dev-CustomerFiles")
            ...
            || checkData[0].Extension.toLowerCase().indexOf("svg") >= 0)
              {//图片
                var pcurl = "/views/devsupplier/pictureview.html?CompId=" + checkData[0].CompId;
                top.winui.window.open({
...
table.on('toolbar(Dev-CustomerFiles)', function (obj) {
 switch (obj.event) {
     case 'add':
        fileEvent.winopen('win_addcustfile','新增附件','/views/devsupplier/compfilebuild.html?CompId='+$devId+'&Ctype=0');
         break;
     case 'batchdel':
         {
             var checkStatus = table.checkStatus('Dev-CustomerFiles');
...
table.on('tool(Dev-CustomerFiles)', function (obj) {
 ...
 switch (obj.event) {
     ...
     fileEvent.deletedata(ids,obj);
         break;
     case 'edit':
        fileEvent.winopen('win_updatecustfile','修改附件','/views/devsupplier/compfilebuild.html?CompId='+$devId+'&Id='+_data.Id);
         break;
     case 'download'://下载
...
table.on('toolbar(Dev-CustomerContact)', function (obj) {
    switch (obj.event) {
        case 'add':
            contactEvent.winopen('win_addcontact','新增联系人','/views/devsupplier/contactbuild.html?CompId='+$devId);
            break;
        case 'batchdel':
            {
...
table.on('tool(Dev-CustomerContact)', function (obj) {
    ...
    switch (obj.event) {
        ...
        $(_data).each(function (index, item) {
            ...
            break;
        case 'edit':
            contactEvent.winopen('win_updatecontact','修改联系人','/views/devsupplier/contactbuild.html?CompId='+$devId+'&Id='+_data.Id);
            break;
        default:
            layer.alert("暂不支持（" + obj.event + "）");
            break;
...
table.on('toolbar(Dev-CustomerDesc)', function (obj) {
 switch (obj.event) {
     case 'add':
        descEvent.winopen('win_adddesc','新增备忘录','/views/devsupplier/descbuild.html?CompId='+$devId);
         break;
     case 'batchdel':
         {
 ...
 switch (obj.event) {
     ...
     $(_data).each(function (index, item) {
       ...
       descEvent.deletedata(ids,obj);
         break;
     case 'edit':
        descEvent.winopen('win_updatedesc','修改备忘录','/views/devsupplier/descbuild.html?CompId='+$devId+'&Id='+_data.Id);
         break;
     default:
         layer.alert("暂不支持（" + obj.event + "）");
         break;
...
/***********************备忘录-end***************************************************************************************************/
                    ...
                    case 'Account':
                        ...
                        {//都是文本编辑框
                            devviewedit.render({
                                ...
                                verify: 'required',
                                ckEl: '#Name',
                                url: devsetter.devbaseurl +'api/DevSupplier/UpdateField'
 ...
 secfield();
//审批历史
 flowapplist.applistInit({ Id: $devId, objType: 1 });
 /******************************************************审批按钮begin*******************************************/

 appflowdata=flowapplist.getappflowinfo({ AppObjId: $devId, ObjType: 1 ,CurrUserId:0});
 ...
 $("#Dev-AppSavebtn").on("click",function(){//审批按钮
    var optiondata={};
    optiondata.prefix="supplier";//前缀
    optiondata.objtype=1;//审批类型
