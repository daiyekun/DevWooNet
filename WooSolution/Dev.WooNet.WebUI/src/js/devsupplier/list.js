layui.config({
   ...
   excel:'soultable/excel'
    ...
    var tburl=devsetter.devbaseurl+"api/DevSupplier/list?ctype=1";
    ...
    table.on('tool(woocustomer)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            wooutil.devajax({
                ...
                data: { funcode: "SupplierDelete",ids:ids.toString() },
                dataType: 'json',
            ...
            wooutil.devajax({//权限判断
                ...
                data: { funcode: "SupplierUpdate",Id:data.Id },
                dataType: 'json',
                success: function (res) {
                    if(res.Tag==-1){
                        top.winui.window.msg(res.msg, {
                        ...
                        devopenwin('win_updatesupplier', data.Id, '修改供应商');
                        layer.close(index);
                    }
            ...
            wooutil.devajax({
                ...
                data: { funcode: "SupplierView",Id:data.Id},
                dataType: 'json',
                success: function (res) {
                    if(res.Tag==-1){
                        top.winui.window.msg(res.msg, {
                        ...
                        var index = layer.load(1);
                         devopenwin('win_viewsupplier', data.Id, '查看供应商',true);
                         layer.close(index);
                    }
    ...
    function reloadTable() {
     ...
     function devopenwin (winid, id, wintitle,Isview) {
        var url = "/views/devsupplier/build.html";
        if (id > 0) {
            url = "/views/devsupplier/build.html?Id=" + id;
        }
        if(Isview){
            url = "/views/devsupplier/view.html?Id=" + id;
        }
    ...
    function devdelete(ids, obj) {
        ...
        top.winui.window.confirm(msg, { icon: 3, title: '删除系统数据' }, function (index) {
            ...
            wooutil.devajax({
                type: 'GET',
                url: devsetter.devbaseurl + 'api/DevSupplier/delete',
          ...
          wooutil.devajax({
            ...
            success: function (res) {
                if(res.Tag==-1){
                    ...
                    devopenwin('win_addsupplier', 0, '新增供应商');
                }
    ...
    $('#tostate').on('click', function(){
        ...
        var url="/views/devsupplier/selstate.html?Id=" 
        + checkStatus.data[0].Id+"&ustate="+checkStatus.data[0].Dstatus
        +"&objtype=1&objcate="+checkStatus.data[0].CompClassId
    ...
    $("#excelexport").on('click',function(){
       var $url= devsetter.devbaseurl+"api/DevSupplier/exportexcel";
