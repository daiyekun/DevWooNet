...
function cleardata(){
    wooutil.devajax({
        type: 'GET',
        url: devsetter.devuserurl + 'api/DevSupplier/cleardata',
    ...
    form.on('submit(Dev-SubmitSave)', function (data) {
        ...
        if (winui.verifyForm(data.elem)) {
           wooutil.devajax({
            type: 'POST',
            url: devsetter.devuserurl + 'api/DevSupplier/companySave',
            data: JSON.stringify(postdata),
            dataType: "json",
    ...
    function closeWin() {
        if ($devId > 0) {
            top.winui.window.close('win_updatesupplier');
        } else {
    ...
    function submitsuccess(json) {
        if (json.code==0) {
            top.winui.window.msg('操作成功', {
                ...
                closeWin();
                top.winui.window.tablelaod({id:'23'});
            });
    ...
    function devSetValues() {
        ...
        if ($devId !== "" && $devId !== undefined) {
            wooutil.devajax({
                type: 'GET',
                url: devsetter.devuserurl + 'api/DevSupplier/showView',
                //async: false,
                data: { Id: $devId },
                dataType: 'json',
                success: function (res) {
    ...
    //供应商类别
     wooutil.getdatadic({ dataenum: 2, selectEl: "#CompClassId" });
     //单位级别
     wooutil.getdatadic({ dataenum: 11, selectEl: "#LevelId" });
     //信用等级
     wooutil.getdatadic({ dataenum: 33, selectEl: "#CareditId" });
    //成立日期
    ...
    //执行赋值表单
    devSetValues();
    form.render(null, 'DEV-SupplierForm');
/***********************基本信息-end***************************************************************************************************/
