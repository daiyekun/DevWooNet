...
layui.config({
   ...
   excel:'soultable/excel'
    ...
    var tburl=devsetter.devbaseurl+"api/DevThird/list?ctype=2";
    ...
    table.on('tool(woocustomer)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'del') { //删除
            wooutil.devajax({
                ...
                data: { funcode: "OtherPartyDelete",ids:ids.toString() },
                dataType: 'json',
            ...
            wooutil.devajax({//权限判断
                ...
                data: { funcode: "OtherPartyUpdate",Id:data.Id },
                dataType: 'json',
                success: function (res) {
                    if(res.Tag==-1){
                        top.winui.window.msg(res.msg, {
                        ...
                        devopenwin('win_updatethird', data.Id, '修改其他对方');
                        layer.close(index);
                    }
            ...
            wooutil.devajax({
                ...
                data: { funcode: "OtherPartyView",Id:data.Id},
                dataType: 'json',
                success: function (res) {
                    if(res.Tag==-1){
                        top.winui.window.msg(res.msg, {
                        ...
                        var index = layer.load(1);
                         devopenwin('win_viewthird', data.Id, '查看其他对方',true);
                         layer.close(index);
                    }
    ...
    function reloadTable() {
     ...
     function devopenwin (winid, id, wintitle,Isview) {
        var url = "/views/devthird/build.html";
        if (id > 0) {
            url = "/views/devthird/build.html?Id=" + id;
        }
        if(Isview){
            url = "/views/devthird/view.html?Id=" + id;
        }
    ...
    function devdelete(ids, obj) {
        ...
        top.winui.window.confirm(msg, { icon: 3, title: '删除系统数据' }, function (index) {
            ...
            wooutil.devajax({
                type: 'GET',
                url: devsetter.devbaseurl + 'api/DevThird/delete',
          ...
          wooutil.devajax({
            ...
            success: function (res) {
                if(res.Tag==-1){
                    ...
                    devopenwin('win_addthird', 0, '新增其他对方');
                }
    ...
    $('#tostate').on('click', function(){
        ...
        var url="/views/devthird/selstate.html?Id=" 
        + checkStatus.data[0].Id+"&ustate="+checkStatus.data[0].Dstatus
        +"&objtype=2&objcate="+checkStatus.data[0].CompClassId
    ...
    $("#excelexport").on('click',function(){
       var $url= devsetter.devbaseurl+"api/DevThird/exportexcel";
