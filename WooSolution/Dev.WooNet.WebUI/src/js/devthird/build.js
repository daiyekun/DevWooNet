...
function cleardata(){
    wooutil.devajax({
        type: 'GET',
        url: devsetter.devuserurl + 'api/DevThird/cleardata',
    ...
    form.on('submit(Dev-SubmitSave)', function (data) {
        ...
        if (winui.verifyForm(data.elem)) {
           wooutil.devajax({
            type: 'POST',
            url: devsetter.devuserurl + 'api/DevThird/companySave',
            data: JSON.stringify(postdata),
            dataType: "json",
    ...
    function closeWin() {
        if ($devId > 0) {
            top.winui.window.close('win_updatethird');
        } else {
            top.winui.window.close('win_addthird');
    ...
    function submitsuccess(json) {
        if (json.code==0) {
            top.winui.window.msg('操作成功', {
                ...
                closeWin();
                top.winui.window.tablelaod({id:'24'});
            });
    ...
    function devSetValues() {
        ...
        if ($devId !== "" && $devId !== undefined) {
            wooutil.devajax({
                type: 'GET',
                url: devsetter.devuserurl + 'api/DevThird/showView',
                //async: false,
                data: { Id: $devId },
                dataType: 'json',
                success: function (res) {
    ...
    //其他对方类别
     wooutil.getdatadic({ dataenum: 4, selectEl: "#CompClassId" });
     //单位级别
     wooutil.getdatadic({ dataenum: 12, selectEl: "#LevelId" });
     //信用等级
     wooutil.getdatadic({ dataenum: 34, selectEl: "#CareditId" });
    //成立日期
...
table.on('toolbar(Dev-CustomerFiles)', function (obj) {
 switch (obj.event) {
     case 'add':
        fileEvent.winopen('win_addcustfile','新增附件','/views/devthird/compfilebuild.html?CompId='+$devId+'&Ctype=0');
         break;
     case 'batchdel':
         {
...
table.on('tool(Dev-CustomerFiles)', function (obj) {
 ...
 switch (obj.event) {
     ...
     fileEvent.deletedata(ids,obj);
         break;
     case 'edit':
        fileEvent.winopen('win_updatecustfile','修改附件','/views/devthird/compfilebuild.html?CompId='+$devId+'&Id='+_data.Id);
         break;
         case 'download'://下载
...
table.on('toolbar(Dev-CustomerContact)', function (obj) {
    switch (obj.event) {
        case 'add':
            contactEvent.winopen('win_addcontact','新增联系人','/views/devthird/contactbuild.html?CompId='+$devId);
            break;
        case 'batchdel':
            {
...
table.on('tool(Dev-CustomerContact)', function (obj) {
    ...
    switch (obj.event) {
        ...
        $(_data).each(function (index, item) {
            ...
            break;
        case 'edit':
            contactEvent.winopen('win_updatecontact','修改联系人','/views/devthird/contactbuild.html?CompId='+$devId+'&Id='+_data.Id);
            break;
        default:
            layer.alert("暂不支持（" + obj.event + "）");
            break;
...
table.on('toolbar(Dev-CustomerDesc)', function (obj) {
 switch (obj.event) {
     case 'add':
        descEvent.winopen('win_adddesc','新增备忘录','/views/devthird/descbuild.html?CompId='+$devId);
         break;
     case 'batchdel':
         {
 ...
 switch (obj.event) {
     ...
     $(_data).each(function (index, item) {
       ...
       descEvent.deletedata(ids,obj);
         break;
     case 'edit':
        descEvent.winopen('win_updatedesc','修改备忘录','/views/devthird/descbuild.html?CompId='+$devId+'&Id='+_data.Id);
         break;
     default:
         layer.alert("暂不支持（" + obj.event + "）");
         break;
