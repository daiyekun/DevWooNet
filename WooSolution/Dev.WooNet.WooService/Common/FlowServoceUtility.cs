...
namespace Dev.WooNet.WooService
{
    ...
    public class FlowServoceUtility
    {
        ...
        /// <returns></returns>
        public  static IList<string>  GetNodeStrIds(SubmitWfRequest submitWfRes, DbContext Db)
        {
            IList<string> nodeIds = new List<string>();
            var listnodeInfos = Db.Set<DevFlowTempNodeInfo>().AsNoTracking().Where(a => a.TempId == submitWfRes.TempId).ToList();
