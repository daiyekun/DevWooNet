...
namespace Dev.WooNet.WebAPI.Areas.DevContract.Controllers.Company
{
    /// <summary>
    /// 合同对方
    /// </summary>
    ...
    public class DevThirdController : ControllerBase
    {
        private IMapper _IMapper;
        private IDevCompanyService _IDevCompanyService;
        private IConfiguration _Configuration;
        private IDevRolePessionService _IDevRolePessionService;
        public DevThirdController(IMapper iMapper, IDevCompanyService iDevCompanyService
        ...
        public IActionResult GetList([FromBody] PgRequestInfo pgInfo, int ctype)
        {
            ...
            prdAnd = prdAnd.And(_IDevRolePessionService.GetCompanyListPermissionExpression("OtherPartyList", userId, departId));
        ...
        public IActionResult CompanySave([FromBody] DevCompanyDTO info)
        {
            ...
            if (info.Id > 0)
            {//修改
                var existname = _IDevCompanyService.GetQueryable(a => a.Name == info.Name && a.Id != info.Id && a.Dtype == 2).Any();
                var existno = _IDevCompanyService.GetQueryable(a => a.Code == info.Code && a.Id != info.Id && a.Dtype == 2).Any();
                if (existname)
                {
            ...
            {
                var existname = _IDevCompanyService.GetQueryable(a => a.Name == info.Name && a.IsDelete != 1 && a.Dtype == 2).Any();
                var existno = _IDevCompanyService.GetQueryable(a => a.Code == info.Code && a.IsDelete != 1&&a.Dtype==2).Any();
                if (existname)
                {
                    result.code = (int)MessageEnums.IsExist;
                    result.msg = "当前名称已经存在";
        ...
        public IActionResult ExportExcel([FromBody] ExportRequestInfo exportRequestInfo)
        {
            ...
            var downInfo = DevExportDataHelper.ExportExcelExtend(exportRequestInfo, "其他对方列表", layPage.data);
