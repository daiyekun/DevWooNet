...
namespace Dev.WooNet.WebAPI.Areas.DevContract.Controllers.Company
{
    ...
    public class DevSupplierController : ControllerBase
    {
        private IMapper _IMapper;
        private IDevCompanyService _IDevCompanyService;
        private IConfiguration _Configuration;
        private IDevRolePessionService _IDevRolePessionService;
        public DevSupplierController(IMapper iMapper, IDevCompanyService iDevCompanyService
        ...
        public IActionResult GetList([FromBody] PgRequestInfo pgInfo,int ctype)
        {
            ...
            prdAnd = prdAnd.And(_IDevRolePessionService.GetCompanyListPermissionExpression("SupplierList", userId, departId));
        ...
        public IActionResult CompanySave([FromBody] DevCompanyDTO info)
        {
            ...
            if (info.Id > 0)
            {//修改
                var existname = _IDevCompanyService.GetQueryable(a => a.Name == info.Name && a.Id != info.Id && a.Dtype == 1).Any();
                var existno = _IDevCompanyService.GetQueryable(a => a.Code == info.Code && a.Id != info.Id && a.Dtype == 1).Any();
                if (existname)
                {
            ...
            {
                var existname = _IDevCompanyService.GetQueryable(a => a.Name == info.Name && a.IsDelete != 1 && a.Dtype == 1).Any();
                var existno = _IDevCompanyService.GetQueryable(a => a.Code == info.Code && a.IsDelete != 1 && a.Dtype == 1).Any();
                if (existname)
                {
                    result.code = (int)MessageEnums.IsExist;
                    result.msg = "当前名称已经存在";
